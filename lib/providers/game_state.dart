...
final Map<String, double> letterWeights = {
  ...
  'P': 1.9, 'Q': 0.10, 'R': 6.0, 'S': 7.3, 'T': 9.1,
  'U': 2.8, 'V': 0.98, 'W': 2.4, 'X': 0.15, 'Y': 2.0, 'Z': 0.07,
};
...
class GameState extends ChangeNotifier {
  ...
  List<String> _buildWeightedLetterList(Map<String, double> weights) {
    final List<String> letters = [];
    weights.forEach((letter, weight) {
      int count = (weight * 10).round(); 
  ...
  void _initializeBoard() {
    final random = Random();
    final weightedLetters = _buildWeightedLetterList(letterWeights);
